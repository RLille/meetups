create_meetup(
  ...
  rmd_params = list(
    ...
    abstract = "R package {bigstatsr} (https://github.com/privefl/bigstatsr) provides functions for fast statistical analysis of large-scale data encoded as matrices. It can handle matrices that are too large to fit in memory. {bigstatsr} is based on a similar format (called `FBM`) as the format `big.matrix` provided by the R package {bigmemory}.
It enables users with a laptop to perform statistical analysis of several dozens of gigabytes of data, and HPC users to handle matrices of hundreds of GB. This package is fast and efficient because of four different reasons. First, {bigstatsr} is memory-efficient
